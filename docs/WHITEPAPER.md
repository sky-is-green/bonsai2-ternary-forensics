...
- **Track A — build our own artifact** (PTQ → QAT/KD). Closed for parity:
  ...
  is corrected to about 2.0× on a clean holdout ([F11](FAILURES.md#f11--t28-holdout-leak-evaluation-on-training-windows)): the original figure was
...
- [**F5**](FAILURES.md#f5--the-last-8-was-localized-to-end-to-end-qat-and-deliberately-not-funded) The last ~8% was localized to end-to-end QAT — every cheaper route was
  ...
  **deliberately not funded** because the result is public and Track B ships it.
  ...
  on its own training windows, and a clean holdout gives ~2.0× ([F11](FAILURES.md#f11--t28-holdout-leak-evaluation-on-training-windows)).
- [**F11**](FAILURES.md#f11--t28-holdout-leak-evaluation-on-training-windows) T28's **1.103× / 90.6%** was measured on windows inside the
  training pool (`build_batches` had no holdout); the clean-holdout figure for
  the same recipe is **~2.0×** at 7000 steps. Retracted; the 1.7B retention must
  be re-baselined.
...
2. **Our own ternary artifact is a training problem — fully localized,
   ...
   1.103× is retracted: its clean-holdout value is about 2.0×
