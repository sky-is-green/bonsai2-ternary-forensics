...
run_one() {  # tag model visible_devices student_dev teacher_dev
  local tag=$1 model=$2 vis=$3 sdev=$4 tdev=$5
  echo "[ladder] start $tag model=$model vis=$vis $(date +%H:%M:%S)"
  HIP_VISIBLE_DEVICES=$vis $PY scripts/pilot/rmd_kd.py $COMMON --lr "$LR" \
    --model-dir "$model" --device "$sdev" --teacher-device "$tdev" \
    --out "artifacts/rmd/ladder-$tag" > "artifacts/rmd/ladder-$tag.log" 2>&1
  echo "[ladder] exit $tag code=$? $(date +%H:%M:%S)"
}

# Phase 1: two smallest rungs in parallel, one per card.
run_one 0.6B "Qwen/Qwen3-0.6B" 0 "cuda:0" "cuda:0" &
run_one 1.7B "$CANARY"         1 "cuda:0" "cuda:0" &
wait

# Phase 2: 4B alone, student on card 0, teacher on card 1.
run_one 4B "Qwen/Qwen3-4B" "0,1" "cuda:0" "cuda:1"
